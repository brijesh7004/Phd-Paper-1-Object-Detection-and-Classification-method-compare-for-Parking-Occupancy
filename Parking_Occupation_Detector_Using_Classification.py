...
import Object_classification as model
model_name = ['resnet50', 'vgg16', 'alexnet', 'mobilenet']
print(model_name)
...
points = [];    points2 = [];   overlap = []; cropPoints = []
mainImg = [];   resizeImg = []
...
def load_config_file():
    if strConfigFileName.get() != "None":
        ...
        global points, points2, cropPoints
        ...
        for item in points:
            pts = []
            for nn in range(1,8,2):                
                pts.append([item[nn], item[nn+1]])
            cropPoints.append(pts)
...
def start_video_file():     
    global mainImg, points, points2, overlap, curTime, totalFrame, imgIdx, skipFrame, vidFrames, detectionData, cropPoints
    isSuccess = True
    
    # model.load_model(detection_Algo)
    while isSuccess:
        name = 'img_cls_{}_{}.jpg'.format(int(imgIdx/skipFrame), detection_Algo)
        dt, name, isCar = [name], './result_cls_image/' + name, []
        ...
        time1 = time.time()
        for pts in cropPoints:
            cropImageFromCanvas(mainImg, pts)
            isCar.append(model.is_object_classify('crop.jpg', detection_Algo, 'Car'))
        ...
        for counter in range(len(points)):
            carDetected, p = isCar[counter], points[counter]            
            treeview.insert('', tk.END, values=[counter+1, 'Occupied' if carDetected else 'Free'])
            drawAreaOnImage(p[1:], 'red' if carDetected else 'green')         
            counter = counter + 1
            dt.append(1 if carDetected else 0)
        ...
        file = open("./result_csv/data_cls_{}.csv".format(detection_Algo), "w")
...
def cropImageFromCanvas(img, point):
    pts = np.array([point])
